...
.get_Zt = function(object) {
  ...
  grps = nlme::getGroups(object$data,
...
.get_Sigma <- function(object) {
  ...
  Lamt <- .to_diag_block(cov_chol, n=nlme::Dim(object$modelStruct$corStruct)$M)
...
.get_dSigma = function(object) {
  ...
  if (!inherits(dsigma_fn, "function")) {
    stop ("Derivatives for correlation structure ",
          cor_struct,
          " is not supported!")
  }

  dSig <- dsigma_fn(object)
  dSig_block = lapply(dSig, .to_diag_block, n=nlme::Dim(object$modelStruct$corStruct)$M)
